            ...
            with st.spinner("Descargando dataset de Kaggle..."):
                ...
                with st.expander("📊 Información del Dataset", expanded=True):
                    ...
                    if len(categorical_columns) > 0:
                        # Agregar sección de gráfico de pastel solo para variables categóricas
                        st.write("**📊 Análisis de Distribución de Variables Categóricas:**")
                        
                        # Seleccionar columna categórica para visualizar
                        selected_column = st.selectbox(
                            "Seleccione una variable categórica para ver su distribución:",
                            categorical_columns
                        )
                        
                        # Crear gráfico de pastel
                        if st.button("Generar Gráfico de Distribución"):
                            # Calcular la distribución de valores
                            value_counts = self.data[selected_column].value_counts()
                            ...
                            plt.pie(value_counts.values, 
                                ...
                                autopct='%1.1f%%',
                                startangle=90)
                            plt.title(f'Distribución de {selected_column}')
                            ...
                            distribution_df = pd.DataFrame({
                                ...
                                'Porcentaje': (value_counts.values / len(self.data) * 100).round(2)
                            })
